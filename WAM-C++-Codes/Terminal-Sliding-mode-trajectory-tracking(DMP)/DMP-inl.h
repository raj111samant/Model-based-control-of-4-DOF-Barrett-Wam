...
template<size_t DOF>
void DMP<DOF>::SetDMPConditions(float* y0, float* goal) {
...
template<size_t DOF>
void DMP<DOF>::GenDMPGaussCenters(float runtime) {
	unsigned int i;
...
template<size_t DOF>
void DMP<DOF>::InitDmpSys(const unsigned int dmps,const unsigned int bfs, float* a, float* b, float runtime, float tolerance)
		{
			dt = 0.002;
			PrevTime = 0.0;
			IsGoal = false;
...
template<size_t DOF>
void DMP<DOF>::CheckDMPGoalOffset() //
{
...
template<size_t DOF>
DMP<DOF>::DMP(const unsigned int dmps, const unsigned int bfs,
		float* a, float* b, float runtime, float tolerance, float* y0,
		float* goal) :
		ref_jp(this, &ref_jp_OutputValue),ref_jv(this, &ref_jv_OutputValue), ref_ja(this, &ref_ja_OutputValue)
{
